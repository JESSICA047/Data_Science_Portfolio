...
function getFileIcon(file) {
  ...
  if (file.name.endsWith(".md"))
    return <VscMarkdown style={{ ...iconStyle, color: "#519975" }} />;
  if (file.name.endsWith(".sql"))
    return <SiMysql style={{ ...iconStyle, color: "#00758f" }} />;
  if (file.name.endsWith(".ipynb"))
    return <SiPython style={{ ...iconStyle, color: "#3572A5" }} />;
...
export default function Projects() {
  ...
  const handleFolderToggle = (section, idx) => {
    const state =
      contentRefs.current[section]?.current &&
    ...
    if (state) {
      setTimeout(() => {
        Flip.from(state, {
          ...
          onLeave: (elements) =>
            gsap.to(elements, { opacity: 0, scale: 0.95, duration: 0.3 }),
        });
      }, 10);
    }
  };
  ...
  useEffect(() => {
    sectionKeys.forEach((section) => {
      ...
      if (
        ...
        const codeBlocks =
          contentRefs.current[section].current.querySelectorAll("pre code");
                            ...
                            {selectedProject.impact && (
                              <div className="project-impact-section">
                                ...
                                {selectedProject.businessValue && (
                                  <div className="business-value-badge">
                                ...
                                {selectedProject.complexity && (
                                  <div
                                    className={`complexity-badge complexity-${selectedProject.complexity.toLowerCase()}`}
                                  >
